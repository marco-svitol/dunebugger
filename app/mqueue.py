...
class NullNATSComm:
    """Null implementation of NATS communication when NATS is disabled"""
    
    def __init__(self, nat_servers=None, client_id=None, subject_root=None, mqueue_handler=None):
        logger.info("NATS messaging is disabled")
        self.is_connected = False
    
    async def start_listener(self):
        """No-op start_listener"""
        logger.debug("NATS disabled - no listener started")
        return None
    
    async def close_listener(self):
        """No-op close_listener"""
        logger.debug("NATS disabled - no listener to close")
    ...
    async def send(self, message: dict, recipient, reply_subject=None):
        """No-op send - messages are dropped silently"""
        logger.debug(f"NATS disabled - dropping message to {recipient}")
        return False
