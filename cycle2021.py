...
def vplaysfx():
    media = vlcinstance.media_new(sfxpath + sfxfile)
    ...
    sfxplayer.play()
    logger.info("Playing sfx :" + sfxpath + sfxfile)
...
def cycle(channel):
    ...
    if GPIO.input(channel) != 1:
        ...
        return
    if channel==GPIOMap["ThreeStateLoop"]: #switching three state to 'loop' mode triggers this function
        ...
        if GPIO.input(GPIOMap["ThreeStateSingle"])==1:
    ...
    #2 Fontana Grossa
    #3 Luci?
    #4 StellaCometa(comando)
    #5 Faro sopra natività traverso
    #6 Fontana Media
    #7 Case 5Volt
    #8 Led Case
    #9 Fisso: Wood+Fuochi+Stellacometa(alim)
    #10 Led Case
    #11 Fisso: Mixer
    #12 -----------------
    #13 Luna
    #14 Faro sopra natività 5V
    #15 -----------------
    #16 ArduinoReset

    #Ch1 ROTTTA####
    #Ch2 Orizzonte Alba/Tramonto
    #Ch3 Alba
    #Ch4 Sole
	...
	#0 sec
        RPiwrite("Dimmer1Reset",1)
        time.sleep(2)
        RPiwrite("ArduinoReset",1) #ArduinoCloseReset
        time.sleep(0.3)
        RPiwrite("ArduinoReset",0) #ArduinoOpenReset
        RPiwrite("Rele1", 1) #Fontane
        RPiwrite("Rele2", 1) #Fontana
        RPiwrite("Rele6", 1) #Fontana
        ...
        timesleep(38)
        #RPiwrite("Rele14",1) #Faro sopra Natività acceso
        timesleep(2)
        RPiwrite("Rele5", 0) #Faro traverso su Natività spento
        timesleep(5) 
        RPiwrite("Rele7", 1) #Case 5V accese
        timesleep(3) 
        RPiwrite("Rele10",1)
        timesleep(2)
        RPiwrite("Rele13",1) #LUNA ON
        timesleep(2)
        RPiwrite("Rele8",1)
        timesleep(3)
        RPiwrite("Rele3",1)
        #40 sec Orizzonte Ch2 fadein speed 30 sec ------------
        timesleep(17)
        RPiwrite("Rele7",0)
        timesleep(18)
        RPiwrite("Rele10",0)
        RPiwrite("Rele13",0) #LUNA OFF
        timesleep(5)  
	...
	# -> tramonto -> notte (in 2 minuti)
        timesleep(123) 
        RPiwrite("Rele13",1) #LUNA ON
        RPiwrite("Rele4",1)  #Stella cometa comando per avvio     
        
	#buio totale
        timesleep(20)
        RPiwrite("Rele4",0) #Stella cometa comando per avvio disinserito
        timesleep(15)
        RPiwrite("Rele3",1) 
        timesleep(8) 
        RPiwrite("Rele7",1)
        ...
        timesleep(10)
        RPiwrite("Rele13",0)
        RPiwrite("Dimmer1Reset",0)
        vstopaudio()
        
        if GPIO.input(GPIOMap["ThreeStateLoop"])!=1:
...
try:
    ...
    chan_releB = [7,8,24,25,23,18]
    chan_contr = [6,13,19]
    chan_ArduinoReset = [14]
    chan_Dimmer1Reset = [15]
    #free GPIOs : 26,12,16,20,21
    GPIOMap={
            "SDA1":chan_I2C[0],
            "SCL1":chan_I2C[1],
            "Rele1":chan_releA[0],
            ...
            "Rele13":chan_releB[4],
            "Rele14":chan_releB[5],
            "Dimmer1Reset":chan_Dimmer1Reset[0],
    ...
    GPIO.setup(chan_Dimmer1Reset, GPIO.OUT,initial=GPIO.HIGH)
    GPIO.setup(chan_contr, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)
    logger.info ("GPIO     : initilized")
    
    # set initial state
    RPiwrite("Rele5",1) # faro Gesù on
    RPiwrite("Rele9",1) # Wood fuochi stellacometa(alim) faroesterno 
    RPiwrite("Rele11",1) #mixer
    ...
    #cycletest(6)
    #cycle(6)
    GPIO.add_event_detect(GPIOMap["StartButton"],GPIO.RISING,callback=cycle,bouncetime=5)
    #GPIO.add_event_detect(GPIOMap["ThreeStateLoop"],GPIO.RISING,callback=cycle,bouncetime=5)
    logger.info ("GPIO     : Callback function 'cycle' binded to event detection on GPIO "+str(GPIOMap["StartButton"]))
    ...
    logger.info ("GPIO     : removing interrupt on GPIO "+str(GPIOMap["StartButton"])+" and cleaning up GPIOs")
    GPIO.remove_event_detect(GPIOMap["StartButton"])
    GPIO.cleanup()
    vstopaudio()
